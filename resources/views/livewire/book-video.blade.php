<div>
    ...
    <form class="w-full">
        ...
        <div class="mb-5">
            ...
            <div class="">
                ...
                <x-text-input wire:model='url' id="url" class="block w-full mt-1" type="text"
                    name="url" placeholder='Youtube Video URL' :value="old('url')" autocomplete="url" />
    ...
    @if ($multiVideos)
        <div class="pt-5">
            <strong class="text-lg font-semibold">Available Videos:</strong>
            <div class="flex flex-wrap gap-4 mt-2">
                @forelse ($multiVideos as $index => $video)
                    @php
                        $url = $video->url; // Assuming url_code contains the full url string
                        preg_match('/src="([^"]+)"/', $url, $matches);
                        $videoSrc = $matches[1] ?? $url; // Fallback to '#' if no src is found
                    @endphp
                    <div wire:key='{{ $video->id }}' class="relative group">
                        <a href='{{ $videoSrc }}'>

                            <img src="{{ asset('assets/images/books/thumb/' . $video->image) }}" alt="Preview Image"
                                class="object-contain max-w-full border rounded-lg shadow-md max-h-40" />
                        </a>
                        <button wire:click="delete({{ $video->id }})" wire:loading.attr="disabled"
                            wire:loading.class="opacity-50 cursor-not-allowed" wire:target="delete"
                            class="absolute p-1 text-white transition-opacity duration-300 ease-in-out bg-red-500 rounded-full opacity-0 -top-5 -right-5 group-hover:opacity-100">
                            <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" fill="none"
                                viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M6 18L18 6M6 6l12 12" />
